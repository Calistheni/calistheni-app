...
test("muscle radar uses the semantic Calistheni blue token", async () => {
  ...
  assert.match(radar, /stroke="var\(--color-workloadScore\)"/);
  assert.match(radar, /fill="var\(--color-workloadScore\)"/);
  ...
  assert.match(radar, /Direct sets/);
  assert.match(radar, /Assisting workload/);
  assert.match(radar, /Workload score/);
  assert.match(radar, /Highest workload/);
  assert.match(radar, /Lowest workload/);
...
test("mobile navigation is one equal-width row with safe-area spacing", async () => {
  ...
  assert.doesNotMatch(appShell, /keepsMobileHeader|pathname === "\/pro"/);
