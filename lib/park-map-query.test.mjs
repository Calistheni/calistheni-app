...
test("admin selection uses a moderate stable zoom and restores heading from a user gesture", async () => {
  const map = await readFile(new URL("components/ParksMap.tsx", root), "utf8");

  assert.match(map, /ADMIN_SELECTED_PARK_ZOOM = 15/);
  assert.match(map, /Math\.max\(map\.getZoom\(\), ADMIN_SELECTED_PARK_ZOOM\)/);
  ...
  assert.match(map, /_addDeviceOrientationListener/);
});
