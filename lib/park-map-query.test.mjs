...
test("public and admin share one stable selection path while location heading remains enabled", async () => {
  const [map, dashboard, adminMap, homePage, userMenu] = await Promise.all([
    readFile(new URL("components/ParksMap.tsx", root), "utf8"),
    readFile(new URL("components/admin/AdminDashboard.tsx", root), "utf8"),
    readFile(new URL("components/admin/AdminParksMap.tsx", root), "utf8"),
    readFile(new URL("components/HomePage.tsx", root), "utf8"),
    readFile(new URL("components/UserMenu.tsx", root), "utf8"),
  ]);
  ...
  assert.match(map, /positionOptions:\s*\{\s*enableHighAccuracy: true/);
  assert.match(map, /trackUserLocation: true/);
  assert.match(map, /showUserLocation: true/);
  assert.match(map, /followUserLocation: false/);
  assert.match(map, /showButton: true/);
  assert.doesNotMatch(map, /_addDeviceOrientationListener =/);
  assert.doesNotMatch(map, /new mapboxgl\.Marker\([\s\S]{0,120}userLocation/);
  ...
  assert.match(
    map,
    /top-\[calc\(env\(safe-area-inset-top\)\+1rem\)\].*right-12 left-24/
  );
  assert.match(
    map,
    /absolute left-4[\s\S]{0,500}bottom-\[calc\(env\(safe-area-inset-bottom\)\+1rem\)\]/
  );
  assert.match(map, /bottom-\[calc\(env\(safe-area-inset-bottom\)\+7rem\)\]/);
  assert.match(map, /addParkControl\?: ReactNode/);
  assert.match(map, /pointer-events-none absolute/);
  assert.match(map, /pointer-events-auto w-fit max-w-full/);
  assert.match(homePage, /addParkControl=\{/);
  assert.doesNotMatch(homePage, /left-\[4\.25rem\]/);
  assert.match(userMenu, /absolute top-\[calc\(env\(safe-area-inset-top\)\+1rem\)\] left-4/);
  assert.match(userMenu, /size="icon-lg"/);
  assert.match(map, /pendingMapResizeRef/);
  assert.match(map, /if \(map\.isMoving\(\)\) \{\s*pendingMapResizeRef\.current = true/);
  assert.match(adminMap, /h-\[32rem\].*shrink-0/);
  assert.match(dashboard, /cursor-pointer border-l-2 hover:bg-muted\/50/);
  assert.match(dashboard, /border-l-primary bg-primary\/5/);
  assert.match(dashboard, /TableHeader className="hidden sm:table-header-group"/);
...
test("cluster navigation preserves one source generation while manual movement keeps Search this area", async () => {
  const map = await readFile(new URL("components/ParksMap.tsx", root), "utf8");

  assert.match(map, /const clusterNavigationPendingRef = useRef\(false\)/);
  assert.match(
    map,
    /const beginClusterNavigation = useCallback\(\(\) => \{[\s\S]*clusterNavigationPendingRef\.current = true;[\s\S]*beginAwayFromUser\(\)/
  );
  assert.match(
    map,
    /if \(clusterNavigationPendingRef\.current\) \{[\s\S]{0,500}clusterNavigationPendingRef\.current = false;[\s\S]{0,100}areaSearchPendingRef\.current = false;[\s\S]{0,100}return;/
  );
  assert.doesNotMatch(
    map,
    /if \(clusterNavigationPendingRef\.current\) \{[\s\S]{0,500}requestViewportParksRef\.current\(\)/
  );
  assert.match(
    map,
    /source\.getClusterExpansionZoom[\s\S]{0,700}beginClusterNavigation\(\)[\s\S]{0,300}map\.easeTo/
  );
  assert.match(
    map,
    /const cancelViewportLoadForClusterNavigation[\s\S]{0,500}viewportLoadIdRef\.current \+= 1;[\s\S]{0,300}controller\.abort\(\)/
  );
  assert.match(
    map,
    /id: "unclustered-point"[\s\S]{0,500}minzoom: detailedParkLayerMinZoom/
  );
  assert.match(
    map,
    /const viewport = requestedViewport \?\? getViewportArea\(map\)/
  );
  assert.match(map, /const loadId = \+\+viewportLoadIdRef\.current/);
  assert.match(map, /const isCurrentLoad = \(\) => viewportLoadIdRef\.current === loadId/);
  assert.match(map, /if \(!isCurrentLoad\(\)\) return/);
  assert.match(
    map,
    /const markManualCameraInteraction = useCallback\(\(\) => \{[\s\S]*areaSearchPendingRef\.current = true/
  );
});

test("the signed-in Parks route uses a full-bleed non-scroll content shell only", async () => {
  const [navigation, globals, home] = await Promise.all([
    readFile(new URL("lib/navigation.ts", root), "utf8"),
    readFile(new URL("app/globals.css", root), "utf8"),
    readFile(new URL("components/HomePage.tsx", root), "utf8"),
  ]);

  assert.match(navigation, /isFullBleedAppRoute[\s\S]*pathname === "\/parks"/);
  assert.match(
    globals,
    /\.app-shell-content-full-bleed \{[\s\S]*overflow: hidden;[\s\S]*padding-bottom: 0;[\s\S]*overscroll-behavior: none/
  );
  assert.match(
    globals,
    /\.app-shell:has\(\[data-active-workout-dock\]\) \.app-shell-content-full-bleed \{\s*padding-bottom: 0/
  );
  assert.match(home, /<main[\s\S]*data-parks-page/);
});
