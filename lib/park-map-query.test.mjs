...
test("cluster navigation loads only the settled cluster viewport while manual movement keeps Search this area", async () => {
  ...
  assert.match(
    map,
    /if \(clusterNavigationPendingRef\.current\) \{[\s\S]*clusterNavigationPendingRef\.current = false;[\s\S]*areaSearchPendingRef\.current = false;[\s\S]*requestViewportParksRef\.current\(\)/
  );
  assert.match(
    map,
