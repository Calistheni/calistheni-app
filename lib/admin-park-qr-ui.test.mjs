...
test("admin park creation uses the same server-owned QR deployment updater", async () => {
  ...
  assert.match(dashboard, /selectedPark=\{selectedPark \?\? selectedParkPreview\}/);
...
test("admin selection uses an archival-aware detail lookup and preserves the list preview", async () => {
  const [service, route, map, dashboard] = await Promise.all([
    readFile(new URL("lib/admin-parks.ts", root), "utf8"),
    readFile(new URL("app/api/admin/parks/[id]/route.ts", root), "utf8"),
    readFile(new URL("components/ParksMap.tsx", root), "utf8"),
    readFile(new URL("components/admin/AdminDashboard.tsx", root), "utf8"),
  ]);

  assert.match(service, /where: \{ id: parkId \}/);
  assert.doesNotMatch(service, /getAdminParkDetail[\s\S]{0,300}getParkVisibilityWhere/);
  assert.doesNotMatch(route, /includeArchived/);
  assert.match(map, /\/api\/admin\/parks\/\$\{parkId\}/);
  assert.match(dashboard, /selectedParkPreview/);
  assert.match(dashboard, /mapSectionRef\.current\?\.scrollIntoView/);
  assert.match(dashboard, /behavior: "smooth"/);
});
