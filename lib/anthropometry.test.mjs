...
import {
  ...
  validateMeasurementCapabilities,
...
test("male Navy estimate uses inches and safely rejects missing or invalid inputs", () => {
  const value = estimateBodyFatPercentage({
    sex: "MALE",
    heightCm: 180,
    neckCm: 38,
    waistAtNavelCm: 85,
  ...
  const expected =
    86.01 * Math.log10((85 - 38) / 2.54) -
    70.041 * Math.log10(180 / 2.54) +
    36.76;
  assert.ok(Math.abs(value - expected) < 1e-12);
  assert.equal(centimetersToInches(2.54), 1);
  const currentCase = estimateBodyFatPercentage({
    sex: "MALE",
    heightCm: 188,
    neckCm: 40,
    waistAtNavelCm: 88,
  });
  assert.ok(Number.isFinite(currentCase));
  assert.ok(currentCase > 0 && currentCase < 60);
  for (const invalid of [
    { sex: "MALE", neckCm: 38, waistAtNavelCm: 85 },
    { sex: "MALE", heightCm: 180, waistAtNavelCm: 85 },
    { sex: "MALE", heightCm: 180, neckCm: 38 },
    { sex: "MALE", heightCm: 180, neckCm: 85, waistAtNavelCm: 85 },
    { sex: "MALE", heightCm: Infinity, neckCm: 38, waistAtNavelCm: 85 },
  ])
    assert.equal(estimateBodyFatPercentage(invalid), null);
});
...
test("female Navy estimate uses inches, requires hips, and never produces non-finite output", () => {
  ...
  const expected =
    163.205 * Math.log10((75 + 100 - 33) / 2.54) -
    97.684 * Math.log10(165 / 2.54) -
    78.387;
  assert.ok(Math.abs(value - expected) < 1e-12);
  assert.equal(
    estimateBodyFatPercentage({
...
test("manual body fat stays separate and takes display precedence", () => {
  assert.deepEqual(bodyFatDisplayValue(18, 21), {
    value: 18,
    source: "manual",
  });
  assert.deepEqual(bodyFatDisplayValue(null, 21), {
    value: 21,
    source: "estimated",
  });
  assert.deepEqual(bodyFatDisplayValue(null, null), {
    value: null,
    source: "unavailable",
  });
});
...
test("catalogue validates all canonical values and rejects malformed entries", () => {
  ...
  assert.equal(
    validateMeasurementValues({ bodyweightKg: "80" }).success,
    false
  );
...
test("Free and Pro capabilities are enforced for direct submissions", () => {
  ...
  assert.deepEqual(free.allowedKeys, [
    "bodyweightKg",
    "heightCm",
    "waistAtNavelCm",
  ]);
  ...
  assert.equal(free.canViewMeasurementCharts, true);
  assert.equal(
    validateMeasurementCapabilities({ hipsCm: 100 }, false).success,
    false
  );
  assert.equal(
    validateMeasurementCapabilities({ shouldersCm: 110 }, false).success,
    false
  );
  assert.equal(
    validateMeasurementCapabilities({ thighCm: 60 }, false).success,
    false
  );
  assert.equal(
    validateMeasurementCapabilities(
      { bodyweightKg: 80, heightCm: 188, waistAtNavelCm: 88 },
      false
    ).success,
    true
  );
  assert.equal(
    validateMeasurementCapabilities({ neckCm: 38 }, false).success,
    false
  );
  assert.equal(
    validateMeasurementCapabilities({ chestCm: 100 }, false).success,
    false
  );
  assert.equal(
    validateMeasurementCapabilities({ bicepsCm: 40 }, false).success,
    false
  );
  assert.equal(
    validateMeasurementCapabilities(
      Object.fromEntries(
        MEASUREMENT_KEYS.map((key) => [key, MEASUREMENT_CATALOGUE[key].min])
      ),
      true
    ).success,
    true
