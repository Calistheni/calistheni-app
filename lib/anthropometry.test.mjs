...
test("male Navy estimate uses inches and safely rejects missing or invalid inputs", () => {
  ...
  const currentCase = estimateBodyFatPercentage({ sex: "MALE", heightCm: 188, neckCm: 40, waistAtNavelCm: 88 });
...
test("Free and Pro capabilities are enforced for direct submissions", () => {
  ...
  assert.deepEqual(free.allowedKeys, ["bodyweightKg", "neckCm", "chestCm", "waistAtNavelCm", "bicepsCm"]);
  ...
  assert.equal(validateMeasurementCapabilities({ thighCm: 60 }, false).success, false);
