...
test("the tracker pre-fills and submits only changed values with an explicit clear mechanism", async () => {
  ...
  assert.match(source, /function MeasurementHistoryCell/);
  assert.match(source, /grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3/);
  assert.match(source, /locked=\{!allowedFields\.has\(field\)\}/);
  assert.match(source, /value == null \? "Not tracked"/);
