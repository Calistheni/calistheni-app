...
test("normalized client data remains valid during server validation", () => {
  const clientResult = partnerInquirySchema.safeParse(validInquiry);
  assert.equal(clientResult.success, true);

  const serverResult = partnerInquirySchema.safeParse(clientResult.data);
  assert.equal(serverResult.success, true);
  assert.equal(serverResult.data?.message, null);
});
