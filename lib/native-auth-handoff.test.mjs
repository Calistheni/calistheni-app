...
test("native handoff uses one-time hashes, official App/Browser plugins, and a secure Auth.js cookie", async () => {
  const [
    ...
    readFile(
      new URL("app/.well-known/apple-app-site-association/route.ts", root),
      "utf8"
    ),
    readFile(
      new URL("components/auth/MobileAuthCallbackPage.tsx", root),
      "utf8"
    ),
  ...
  assert.match(browserStart, /external_login_context_cleared/);
  ...
  assert.match(handoff, /handledCodesRef = useRef\(new Set<string>\(\)\)/);
  assert.match(handoff, /handledCodesRef\.current\.has\(handoff\.code\)/);
  assert.match(handoff, /handledCodesRef\.current\.add\(handoff\.code\)/);
