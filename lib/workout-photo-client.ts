...
export async function compressWorkoutPhoto(file: File) {
  // Camera captures can return an empty/octet-stream MIME despite valid JPEG
  // bytes. Skip the browser fast path and let the byte-validating Node fallback
  // inspect the original rather than failing workout completion.
