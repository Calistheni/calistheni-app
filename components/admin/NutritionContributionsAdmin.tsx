    ...
    {history.foods.map((food) => <Card key={food.id}>
      <CardHeader className="flex-row items-start justify-between gap-3">
        <div className="min-w-0 space-y-1">
          ...
          <div className="space-y-1 text-xs text-muted-foreground"><p>{food.contributionKind === "BARCODE_PRODUCT" ? `Barcode product${food.submissionMethod ? ` · ${food.submissionMethod === "AI_LABEL" ? "AI label scan" : "Manual"}` : ""}` : "Generic food"}</p><div className="flex items-center gap-1"><span>Barcode</span><code className="rounded bg-muted px-1.5 py-0.5 font-mono font-semibold text-foreground">{food.barcode ?? "—"}</code>{food.barcode ? <Button size="icon" variant="ghost" className="size-6" aria-label={`Copy barcode ${food.barcode}`} onClick={() => void copyBarcode(food.barcode!)}><Copy className="size-3.5" /></Button> : null}</div></div>
