...
export async function PATCH(
  ...
  const clearValidation = validateStoredMeasurementCapabilities(
    Object.fromEntries(
      clearFields.map((field) => [field, 1])
    ) as MeasurementSnapshotValues,
    isPro
  );
  if (!clearValidation.success)
    return createJsonValidationErrorResponse(
      "Some measurements require Pro.",
      validationErrors(clearValidation.errors)
    );
  const clearData = Object.fromEntries(
    clearFields.map((field) => [field, null])
  );
  try {
    const result = await prisma.bodyMeasurementEntry.updateMany({
      where: { id, userId },
      data: {
        ...submitted,
        ...clearData,
        ...(measuredAt ? { measuredAt } : {}),
        ...(hasSubmittedNote ? { note: note ?? null } : {}),
      },
    });
    return result.count
      ? NextResponse.json({ ok: true })
      : createJsonErrorResponse("Measurement not found.", 404);
  } catch (error) {
    console.error(error);
...
export async function DELETE(
  ...
  try {
    const deleted = await prisma.$transaction(async (tx) => {
      const entry = await tx.bodyMeasurementEntry.findFirst({
        where: { id, userId },
        select: { id: true },
      });
      if (!entry) return false;

      await tx.bodyMeasurementEntry.delete({ where: { id: entry.id } });
      const remaining = await tx.bodyMeasurementEntry.findMany({
        where: { userId },
        orderBy: [{ measuredAt: "desc" }, { createdAt: "desc" }],
      });
      const next = latestMeasurementSnapshot(remaining);
      await tx.user.update({
        where: { id: userId },
        data: { bodyweightKg: next.bodyweightKg ?? null },
      });
      return true;
    });
    return deleted
