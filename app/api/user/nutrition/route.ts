...
import { calculateNutritionGoalProgress } from "@/lib/nutrition/goals";
...
function values(record: Record<string, unknown>) {
  return Object.fromEntries(
    fields.map((key) => [
      key,
      record[key] == null ? undefined : Number(record[key]),
    ])
  ) as Record<(typeof fields)[number], number | undefined>;
}
function snapshotData(
  food: { currentRevision: Record<string, unknown> },
  grams: number
) {
  const revision = food.currentRevision;
  const calculated = snapshotForFood(values(revision), grams);
  return {
    foodRevisionId: String(revision.id),
    ...
    ...Object.fromEntries(
      fields.map((key) => [`${key}Snapshot`, calculated[key] ?? null])
    ),
  };
}
  ...
  try {
    const [entries, goal] = await Promise.all([
      prisma.nutritionEntrySnapshot.findMany({
        where: { userId, loggedFor: nutritionDate(parsed.data) },
        include: {
          food: {
            include: {
      ...
      getNutritionGoalForDate(userId, parsed.data),
    ]);
    const serializedEntries = entries.map(serializeNutritionEntry);
    const totals = nutritionTotals(serializedEntries);
    return NextResponse.json(
      {
        entries: serializedEntries,
        goal,
        progress: goal ? calculateNutritionGoalProgress(totals, goal) : null,
        // Keep this response field while callers transition to the explicit goal.
        targets: goal,
      },
      { headers: { "Cache-Control": "private, no-store, max-age=0" } }
    );
  } catch (error) {
    console.error("NUTRITION_LOG_GET_FAILED", error);
...
export async function POST(request: Request) {
  ...
  const parsed = nutritionEntrySchema.safeParse(body);
  if (!parsed.success)
  ...
  try {
    ...
    if (!food?.currentRevision)
      return createJsonErrorResponse("Food is unavailable for logging.", 404);
    const created = await prisma.nutritionEntrySnapshot.create({
      data: {
        userId,
        foodId: food.id,
        ...
        ...snapshotData(food as never, parsed.data.gramsConsumed),
      },
    });
    const entry = await prisma.nutritionEntrySnapshot.findUniqueOrThrow({
      where: { id: created.id },
      include: {
        food: {
          include: {
    ...
    return NextResponse.json(serializeNutritionEntry(entry), { status: 201 });
  } catch (error) {
    console.error("NUTRITION_LOG_CREATE_FAILED", error);
    return createInternalServerErrorResponse();
  }
}
