...
const schema = z.object({ action: z.enum(["approve", "reject"]), name: z.string().trim().min(2).max(120).optional(), caloriesKcal: z.number().min(0).max(2000).optional(), proteinGrams: z.number().min(0).max(200).optional(), carbohydrateGrams: z.number().min(0).max(300).optional(), fatGrams: z.number().min(0).max(200).optional() }).strict();
  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      if (approved && (parsed.data.name || parsed.data.caloriesKcal !== undefined || parsed.data.proteinGrams !== undefined || parsed.data.carbohydrateGrams !== undefined || parsed.data.fatGrams !== undefined)) {
        const revision = await tx.foodRevision.create({ data: { foodId: food.id, revisionNumber: (food.revisions[0]?.revisionNumber ?? 0) + 1, reason: FoodRevisionReason.ADMIN_CORRECTION, source: FoodSource.CALISTHENI, sourceExternalId: food.sourceExternalId, name: edits.name, nutritionBasisGrams: 100, caloriesKcal: edits.caloriesKcal, proteinGrams: edits.proteinGrams, carbohydrateGrams: edits.carbohydrateGrams, fatGrams: edits.fatGrams, confidenceScore: 0.9, verificationStatus, normalizedDataChecksum: `admin:${Date.now()}` } });
        return tx.food.update({ where: { id: food.id }, data: { ...edits, contributionStatus: status, verificationStatus, currentRevisionId: revision.id } });
      }
      return tx.food.update({ where: { id: food.id }, data: { contributionStatus: status, verificationStatus } });
    });
